...
class DarkTheme {
    ...
    static Brushes := Map()
    /** @type {Map} Cached 1px GDI pen handles keyed by color name */
    static Pens := Map()
    ...
    static __New() {
        for name, color in this.Colors {
            ...
            this.Pens[name] := DllCall("gdi32\CreatePen", "Int", 0, "Int", 1, "UInt", this.RGBtoBGR(color), "Ptr")
        }
        if !this._exitRegistered {
            OnExit((*) {
                try _DarkSlider.Shutdown()
                try DarkTheme.Cleanup()
                return 0
            })
    ...
    /**
     * Gets a cached 1px GDI pen handle for the specified color.
     * @param {String} name - Color name from Colors map
     * @returns {Ptr} GDI pen handle or 0 if not found
     */
    static GetPen(name) => this.Pens.Has(name) ? this.Pens[name] : 0

    /**
     * Updates a theme color and recreates its brush + pen.
    ...
    static SetColor(name, value) {
        ...
        if this.Pens.Has(name)
            DllCall("DeleteObject", "Ptr", this.Pens[name])
        this.Colors[name] := value
        this.Brushes[name] := DllCall("gdi32\CreateSolidBrush", "UInt", this.RGBtoBGR(value), "Ptr")
        this.Pens[name] := DllCall("gdi32\CreatePen", "Int", 0, "Int", 1, "UInt", this.RGBtoBGR(value), "Ptr")
    }
...
class _DarkListView extends Gui.ListView {
    ...
    static ListViewProc(targetHwnd, hwnd, msg, wParam, lParam) {
        ...
        if hwnd != targetHwnd
            return Subclass.CallOriginal(this.OldProcs.Get(targetHwnd, 0), hwnd, msg, wParam, lParam)
        ...
        if msg = WM_NCPAINT {
            ...
            result := Subclass.CallOriginal(this.OldProcs.Get(targetHwnd, 0), hwnd, msg, clippedRgn ? clippedRgn : wParam, lParam)
        ...
        if msg = WM_NCMOUSEMOVE {
            result := Subclass.CallOriginal(this.OldProcs.Get(targetHwnd, 0), hwnd, msg, wParam, lParam)
...
class _DarkSlider extends Gui.Slider {
    ...
    static ApplyDarkMode(slider) {
        ...
        DllCall("InvalidateRect", "Ptr", slider.Hwnd, "Ptr", 0, "Int", 1)
    }
...
class _DarkGroupBox {
    ...
    static Paint(hwnd) {
        ...
        oPen  := DllCall("SelectObject", "Ptr", hdc, "Ptr", DarkTheme.GetPen("Border"), "Ptr")
...
class DarkGui extends Gui {
     ...
     */
    __Delete() {
        ; Wrapped end-to-end: alpha.28 makes Map[missing] throw and lambdas return unset
        ; by default, both of which can surface as exit code 5 during teardown when this
        ; runs after the underlying window/handles are already gone.
        try {
