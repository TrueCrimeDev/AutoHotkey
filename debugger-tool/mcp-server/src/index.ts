...
async function main() {
  ...
  // Start listening for AutoHotkey (legacy TCP mode).
  // A bind failure (e.g. another debugger already holding port 9000) must NOT be
  // fatal: launch_script drives scripts over stdio without the port, the
  // shell-based tools (ast_outline, get_source_context, ...) never touch the
  // DBGp client, and every debugger tool already guards on client.isConnected()
  ...
  try {
    ...
    console.error('Legacy TCP debugging is unavailable; launch_script (stdio) and the source tools');
    console.error('(ast_outline, get_source_context, ...) still work. Free the port and restart to');
    console.error('enable /Debug connections.');
  }
