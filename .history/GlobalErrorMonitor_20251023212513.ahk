...
; Options: 1 = Info icon
TrayTip("AHK Error Monitor Started", "Monitoring: " . MonitorConfig.logDirectory, "1")
...
ShowErrorNotification(errorInfo) {
    ...
    ; Options: 3 = Error icon, duration is set separately in TrayTip
...
ShowRecentErrors() {
    ...
    if (recentErrors.Length == 0) {
        ; Options: i = info icon
        MsgBox("No errors detected yet.", "Recent Errors", "i")
        return
    }
    ...
    ; Show in message box
    ; Options: ! = warning icon, T0 = no timeout
    MsgBox(errorList, "Recent Errors", "! T0")
}
...
ToggleMonitoring() {
    ...
    if (isMonitoring) {
        ...
        A_IconTip := "AHK Error Monitor - Watching for errors..."
        ; Options: 1 = Info icon
        TrayTip("Monitoring Resumed", "Now watching for errors", "1")
    } else {
        A_TrayMenu.Rename("Pause Monitoring", "Resume Monitoring")
        A_IconTip := "AHK Error Monitor - Paused"
        ; Options: 2 = Warning icon
        TrayTip("Monitoring Paused", "Not watching for errors", "2")
    }
}
