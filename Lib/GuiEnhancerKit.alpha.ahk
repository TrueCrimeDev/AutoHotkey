...
class GuiExt extends Gui
{
     ...
     * upper-left and lower-right corners. Backed by alpha.30's native typed
     * `Struct`: the engine computes field offsets, alignment and size, so the
     * old hand-rolled `__Struct` / NumGet / NumPut / DefineProp machinery is gone.
     *
     * @example  ; Fresh struct, filled by a Win32 call (pass it straight as "Ptr"):
     * DllCall("GetWindowRect", "Ptr", WinExist("A"), "Ptr", rc := GuiExt.RECT(), "Void")
     * MsgBox(rc.left " " rc.top " " rc.right " " rc.bottom)
     *
     * @example  ; Seed selected fields from an object:
     * rc := GuiExt.RECT({top: 10, bottom: 69})   ; L0 / T10 / R0 / B69
     *
     * @example  ; Read a RECT that already lives at an address (e.g. an lParam):
     * rc := GuiExt.RECT(lParam)        ; snapshot copy of the 16 bytes
     * rc := GuiExt.RECT.At(lParam)     ; or a live overlay (reflects writes)
     *
     * @link [Learn more on MSDN](https://learn.microsoft.com/en-us/windows/win32/api/windef/ns-windef-rect)
     */
    Struct RECT
    {
        /** @prop {Int32} left   x-coordinate of the upper-left corner. */
        left:   Int32
        /** @prop {Int32} top    y-coordinate of the upper-left corner. */
        top:    Int32
        /** @prop {Int32} right  x-coordinate of the lower-right corner. */
        right:  Int32
        /** @prop {Int32} bottom y-coordinate of the lower-right corner. */
        bottom: Int32

        /** @prop {Int32} width  Rect width  (right - left). */
        width  => this.right  - this.left
        /** @prop {Int32} height Rect height (bottom - top). */
        height => this.bottom - this.top

        /**
         * @param {object|integer} [objOrAddress] *Optional.* Seed the struct: an
         * object copies matching `{left,top,right,bottom}` props; a numeric address
         * copies the 16 bytes found there. Omit for a zeroed RECT. For a *live* view
         * onto existing memory use `GuiExt.RECT.At(address)` instead.
         */
        __New(objOrAddress?)
        {
            if !IsSet(objOrAddress)
                return
            if IsObject(objOrAddress) {
                for prop, value in objOrAddress.OwnProps()
                    if this.HasProp(prop)
                        this.%prop% := value
            } else if IsNumber(objOrAddress)
                DllCall("RtlMoveMemory", "Ptr", this.Ptr, "Ptr", objOrAddress, "Ptr", this.Size, "Void")
        }
    }
    ...
    SetBorderless(border := 6, dragWndFunc := "", cxLeftWidth?, cxRightWidth?, cyTopHeight?, cyBottomHeight?)
    {
        ...
        ExtendFrameIntoClientArea(cxLeftWidth?, cxRightWidth?, cyTopHeight?, cyBottomHeight?)
        {
            ...
            DllCall("Dwmapi\DwmExtendFrameIntoClientArea", "Ptr", this.hWnd, "Ptr", margin, "Void")
        }
    }
     ...
     * @returns {RECT}
     */
    GetWindowRect() => (DllCall("GetWindowRect", "ptr", this.hwnd, "ptr", _rc := GuiExt.RECT(), "Void"), _rc)
     ...
     * @returns {RECT}
     */
    GetClientRect() => (DllCall("GetClientRect", "ptr", this.hwnd, "ptr", _rc := GuiExt.RECT(), "Void"), _rc)
    ...
    SetDarkMenu()
    {
        ...
        DllCall(SetPreferredAppMode, "int", 1, "Void")
        ...
        */
        ...
        SetRounded(corner := 9)
        {
            ...
            SIZING(ctrl, wParam?, lParam?, msg?)
            {
                ...
                DllCall("SetWindowRgn", "ptr", ctrl.hWnd, "ptr", rcRgn, "int", 1, "Void")
                ctrl.Redraw()
                DllCall('Gdi32\DeleteObject', 'ptr', rcRgn, 'Void')
            }
        }
...
Example_GuiExt()
{
    ...
    if (VerCompare(A_OSVersion, "10.0.22000") >= 0)
    {
        ...
        ; Mica (Alt) backdrop is *translucent* — it tints the window's edges with
        ; whatever sits behind it on the desktop, so the frame appears to "bleed".
        ; Left disabled so the window stays fully opaque on any background. Uncomment
        ; the two lines below to showcase Mica. (Requires win 11 build 22600.)
    ...
    ; Set the borderless. Omit the four margin args so the DWM frame extends across
    ; the whole window (sheet-of-glass). Passing fixed insets like 500,500,500,500
    ; leaves an un-extended opaque rectangle in the centre that grows as you enlarge
    ; the window — the "dark box" artifact. The 6px arg is the resize hit-test border.
    myGui.SetBorderless(6, BorderlessCallback)
    ...
    myEdit.OnEvent("Focus", (gCtrl, *) => (DllCall("HideCaret", "ptr", gCtrl.hWnd, "Void"), gCtrl.SendMsg(EN_KILLFOCUS)))
    ...
    SetCursor(GuiCtrlObj, wParam, lParam, Msg) {
        ...
        DllCall("SetCursor", "ptr", hCursor, "Void")
        return 0
    }
}
