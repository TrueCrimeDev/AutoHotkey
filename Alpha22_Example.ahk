/*
darkmode_alpha.ahk — Dark mode GUI framework for AutoHotkey v2.1-alpha.22+

Alpha.22 features used:
  - Native Struct keyword for Win32 structures (RECT, NMHDR, NMCUSTOMDRAW)
  - Struct.At(ptr) for zero-copy pointer views (replaces StructFromPtr)
  - DefineProp() top-level function for prototype extensions
  - Type() returning "unset" for optional parameter dispatch

Usage:
  #Include darkmode_alpha.ahk
...
/**
 * Installs SetDarkMode() on Gui control prototypes.
 * Uses alpha.22 top-level DefineProp() function for cleaner prototype extension.
 */
class DarkPrototypes {
    static __New() {
        ; alpha.22: DefineProp() as top-level function — cleaner than obj.DefineProp()
        ; when extending prototypes you don't own
        DefineProp(Gui.Edit.Prototype, "SetDarkMode", { Call: (ctrl) {
        ...
        DefineProp(Gui.CheckBox.Prototype, "SetDarkMode", { Call: (ctrl) {
            DllCall("uxtheme\SetWindowTheme", "Ptr", ctrl.Hwnd, "Str", "DarkMode_Explorer", "Ptr", 0)
        }})

        DefineProp(Gui.Radio.Prototype, "SetDarkMode", { Call: (ctrl) {
            DllCall("uxtheme\SetWindowTheme", "Ptr", ctrl.Hwnd, "Str", "DarkMode_Explorer", "Ptr", 0)
        }})

        DefineProp(Gui.TreeView.Prototype, "SetDarkMode", { Call: (ctrl) {
...
class _DarkListView extends Gui.ListView {
    ...
    static ClipArrowRegion(hwnd, wParam) {
        ...
        if !DllCall("GetScrollBarInfo", "Ptr", hwnd, "Int", OBJID_VSCROLL, "Ptr", sbi)
            return 0
        if sbi.rgstate0 & 0x8000  ; STATE_SYSTEM_INVISIBLE
            return 0
...
class _DarkGroupBox {
    ...
    static Paint(hwnd) {
        ...
        if StrLen(text) > 0 {
            gapRc := RECT()
            gapRc.left := textX - 2, gapRc.top := borderY - 1, gapRc.right := textX + textW + 2, gapRc.bottom := borderY + 1
        ...
        loop count {
            ...
            if (i = selIdx) {
                ...
                DllCall("RoundRect", "Ptr", hdc,
                    ...
                    "Int", 6, "Int", 6)
                squareRc := RECT()
                squareRc.left := left+2, squareRc.top := bottom-6, squareRc.right := right-1, squareRc.bottom := bottom+1
    ...
    ShowPopupMenu(hPopup, x, *) {
        ...
        for item in this.menuItems {
            if item["popup"] = hPopup {
                ...
                popupX := ctrlRect.left
                popupY := ctrlRect.bottom
...
class DarkGui extends Gui {
    ...
    Add(controlType, options := "", content?) {
        ; alpha.22: Type() returns "unset" — use for logging/debugging optional params
    ...
    _AddRadio(options, text?) {
        ...
        radio.TextCtrl := txt
        ; alpha.22: DefineProp() as top-level function
        DefineProp(radio, "Text", {
